...
class ACTEncoder(nn.Module): 
    def __init__(self, 
        ...
        encoder_layer = nn.TransformerEncoderLayer(d_model=self.hidden_dims,
        ...
        self.act_encoder = nn.TransformerEncoder(encoder_layer=encoder_layer, num_layers=self.num_layers)
    ...
    def forward(self, x: torch.Tensor): 
        ...
        if x.shape[1] != self.num_proprio_tokens + self.num_img_tokens:
            raise ValueError(f"input tokens to ACT encoder != {self.num_img_tokens + self.num_proprio_tokens}: {x.shape[1]}")
